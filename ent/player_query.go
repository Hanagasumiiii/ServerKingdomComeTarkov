...
func (pq *PlayerQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Player, init func(*Player), assign func(*Player, *User)) error {
	...
	for i := range nodes {
		if nodes[i].user_player == nil {
			continue
		}
		fk := *nodes[i].user_player
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_player" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
