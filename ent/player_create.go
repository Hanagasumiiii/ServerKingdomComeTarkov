// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"
	"errors"
	"fmt"

	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
	"github.com/Hanagasumiiii/ServerKingdomComeTarkov/ent/player"
	"github.com/Hanagasumiiii/ServerKingdomComeTarkov/ent/user"
)

// PlayerCreate is the builder for creating a Player entity.
type PlayerCreate struct {
	config
	mutation *PlayerMutation
	hooks    []Hook
}

// SetMana sets the "mana" field.
func (pc *PlayerCreate) SetMana(i int) *PlayerCreate {
	pc.mutation.SetMana(i)
	return pc
}

// SetNillableMana sets the "mana" field if the given value is not nil.
func (pc *PlayerCreate) SetNillableMana(i *int) *PlayerCreate {
	if i != nil {
		pc.SetMana(*i)
	}
	return pc
}

// SetHp sets the "hp" field.
func (pc *PlayerCreate) SetHp(i int) *PlayerCreate {
	pc.mutation.SetHp(i)
	return pc
}

// SetNillableHp sets the "hp" field if the given value is not nil.
func (pc *PlayerCreate) SetNillableHp(i *int) *PlayerCreate {
	if i != nil {
		pc.SetHp(*i)
	}
	return pc
}

// SetPositionX sets the "position_x" field.
func (pc *PlayerCreate) SetPositionX(f float64) *PlayerCreate {
	pc.mutation.SetPositionX(f)
	return pc
}

// SetNillablePositionX sets the "position_x" field if the given value is not nil.
func (pc *PlayerCreate) SetNillablePositionX(f *float64) *PlayerCreate {
	if f != nil {
		pc.SetPositionX(*f)
	}
	return pc
}

// SetPositionY sets the "position_y" field.
func (pc *PlayerCreate) SetPositionY(f float64) *PlayerCreate {
	pc.mutation.SetPositionY(f)
	return pc
}

// SetNillablePositionY sets the "position_y" field if the given value is not nil.
func (pc *PlayerCreate) SetNillablePositionY(f *float64) *PlayerCreate {
	if f != nil {
		pc.SetPositionY(*f)
	}
	return pc
}

// SetPositionZ sets the "position_z" field.
func (pc *PlayerCreate) SetPositionZ(f float64) *PlayerCreate {
	pc.mutation.SetPositionZ(f)
	return pc
}

// SetNillablePositionZ sets the "position_z" field if the given value is not nil.
func (pc *PlayerCreate) SetNillablePositionZ(f *float64) *PlayerCreate {
	if f != nil {
		pc.SetPositionZ(*f)
	}
	return pc
}

// SetInventory sets the "inventory" field.
func (pc *PlayerCreate) SetInventory(s []string) *PlayerCreate {
	pc.mutation.SetInventory(s)
	return pc
}

// SetUserID sets the "user" edge to the User entity by ID.
func (pc *PlayerCreate) SetUserID(id int) *PlayerCreate {
	pc.mutation.SetUserID(id)
	return pc
}

// SetUser sets the "user" edge to the User entity.
func (pc *PlayerCreate) SetUser(u *User) *PlayerCreate {
	return pc.SetUserID(u.ID)
}

// Mutation returns the PlayerMutation object of the builder.
func (pc *PlayerCreate) Mutation() *PlayerMutation {
	return pc.mutation
}

// Save creates the Player in the database.
func (pc *PlayerCreate) Save(ctx context.Context) (*Player, error) {
	pc.defaults()
	return withHooks(ctx, pc.sqlSave, pc.mutation, pc.hooks)
}

// SaveX calls Save and panics if Save returns an error.
func (pc *PlayerCreate) SaveX(ctx context.Context) *Player {
	v, err := pc.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (pc *PlayerCreate) Exec(ctx context.Context) error {
	_, err := pc.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (pc *PlayerCreate) ExecX(ctx context.Context) {
	if err := pc.Exec(ctx); err != nil {
		panic(err)
	}
}

// defaults sets the default values of the builder before save.
func (pc *PlayerCreate) defaults() {
	if _, ok := pc.mutation.Mana(); !ok {
		v := player.DefaultMana
		pc.mutation.SetMana(v)
	}
	if _, ok := pc.mutation.Hp(); !ok {
		v := player.DefaultHp
		pc.mutation.SetHp(v)
	}
	if _, ok := pc.mutation.PositionX(); !ok {
		v := player.DefaultPositionX
		pc.mutation.SetPositionX(v)
	}
	if _, ok := pc.mutation.PositionY(); !ok {
		v := player.DefaultPositionY
		pc.mutation.SetPositionY(v)
	}
	if _, ok := pc.mutation.PositionZ(); !ok {
		v := player.DefaultPositionZ
		pc.mutation.SetPositionZ(v)
	}
}

// check runs all checks and user-defined validators on the builder.
func (pc *PlayerCreate) check() error {
	if _, ok := pc.mutation.Mana(); !ok {
		return &ValidationError{Name: "mana", err: errors.New(`ent: missing required field "Player.mana"`)}
	}
	if _, ok := pc.mutation.Hp(); !ok {
		return &ValidationError{Name: "hp", err: errors.New(`ent: missing required field "Player.hp"`)}
	}
	if _, ok := pc.mutation.PositionX(); !ok {
		return &ValidationError{Name: "position_x", err: errors.New(`ent: missing required field "Player.position_x"`)}
	}
	if _, ok := pc.mutation.PositionY(); !ok {
		return &ValidationError{Name: "position_y", err: errors.New(`ent: missing required field "Player.position_y"`)}
	}
	if _, ok := pc.mutation.PositionZ(); !ok {
		return &ValidationError{Name: "position_z", err: errors.New(`ent: missing required field "Player.position_z"`)}
	}
	if len(pc.mutation.UserIDs()) == 0 {
		return &ValidationError{Name: "user", err: errors.New(`ent: missing required edge "Player.user"`)}
	}
	return nil
}

func (pc *PlayerCreate) sqlSave(ctx context.Context) (*Player, error) {
	if err := pc.check(); err != nil {
		return nil, err
	}
	_node, _spec := pc.createSpec()
	if err := sqlgraph.CreateNode(ctx, pc.driver, _spec); err != nil {
		if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{msg: err.Error(), wrap: err}
		}
		return nil, err
	}
	id := _spec.ID.Value.(int64)
	_node.ID = int(id)
	pc.mutation.id = &_node.ID
	pc.mutation.done = true
	return _node, nil
}

func (pc *PlayerCreate) createSpec() (*Player, *sqlgraph.CreateSpec) {
	var (
		_node = &Player{config: pc.config}
		_spec = sqlgraph.NewCreateSpec(player.Table, sqlgraph.NewFieldSpec(player.FieldID, field.TypeInt))
	)
	if value, ok := pc.mutation.Mana(); ok {
		_spec.SetField(player.FieldMana, field.TypeInt, value)
		_node.Mana = value
	}
	if value, ok := pc.mutation.Hp(); ok {
		_spec.SetField(player.FieldHp, field.TypeInt, value)
		_node.Hp = value
	}
	if value, ok := pc.mutation.PositionX(); ok {
		_spec.SetField(player.FieldPositionX, field.TypeFloat64, value)
		_node.PositionX = value
	}
	if value, ok := pc.mutation.PositionY(); ok {
		_spec.SetField(player.FieldPositionY, field.TypeFloat64, value)
		_node.PositionY = value
	}
	if value, ok := pc.mutation.PositionZ(); ok {
		_spec.SetField(player.FieldPositionZ, field.TypeFloat64, value)
		_node.PositionZ = value
	}
	if value, ok := pc.mutation.Inventory(); ok {
		_spec.SetField(player.FieldInventory, field.TypeJSON, value)
		_node.Inventory = value
	}
	if nodes := pc.mutation.UserIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2O,
			Inverse: true,
			Table:   player.UserTable,
			Columns: []string{player.UserColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(user.FieldID, field.TypeInt),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_node.user_player = &nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
	return _node, _spec
}

// PlayerCreateBulk is the builder for creating many Player entities in bulk.
type PlayerCreateBulk struct {
	config
	err      error
	builders []*PlayerCreate
}

// Save creates the Player entities in the database.
func (pcb *PlayerCreateBulk) Save(ctx context.Context) ([]*Player, error) {
	if pcb.err != nil {
		return nil, pcb.err
	}
	specs := make([]*sqlgraph.CreateSpec, len(pcb.builders))
	nodes := make([]*Player, len(pcb.builders))
	mutators := make([]Mutator, len(pcb.builders))
	for i := range pcb.builders {
		func(i int, root context.Context) {
			builder := pcb.builders[i]
			builder.defaults()
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				mutation, ok := m.(*PlayerMutation)
				if !ok {
					return nil, fmt.Errorf("unexpected mutation type %T", m)
				}
				if err := builder.check(); err != nil {
					return nil, err
				}
				builder.mutation = mutation
				var err error
				nodes[i], specs[i] = builder.createSpec()
				if i < len(mutators)-1 {
					_, err = mutators[i+1].Mutate(root, pcb.builders[i+1].mutation)
				} else {
					spec := &sqlgraph.BatchCreateSpec{Nodes: specs}
					// Invoke the actual operation on the latest mutation in the chain.
					if err = sqlgraph.BatchCreate(ctx, pcb.driver, spec); err != nil {
						if sqlgraph.IsConstraintError(err) {
							err = &ConstraintError{msg: err.Error(), wrap: err}
						}
					}
				}
				if err != nil {
					return nil, err
				}
				mutation.id = &nodes[i].ID
				if specs[i].ID.Value != nil {
					id := specs[i].ID.Value.(int64)
					nodes[i].ID = int(id)
				}
				mutation.done = true
				return nodes[i], nil
			})
			for i := len(builder.hooks) - 1; i >= 0; i-- {
				mut = builder.hooks[i](mut)
			}
			mutators[i] = mut
		}(i, ctx)
	}
	if len(mutators) > 0 {
		if _, err := mutators[0].Mutate(ctx, pcb.builders[0].mutation); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

// SaveX is like Save, but panics if an error occurs.
func (pcb *PlayerCreateBulk) SaveX(ctx context.Context) []*Player {
	v, err := pcb.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (pcb *PlayerCreateBulk) Exec(ctx context.Context) error {
	_, err := pcb.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (pcb *PlayerCreateBulk) ExecX(ctx context.Context) {
	if err := pcb.Exec(ctx); err != nil {
		panic(err)
	}
}
