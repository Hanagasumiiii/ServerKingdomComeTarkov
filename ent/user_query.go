...
func (uq *UserQuery) QueryPlayer() *PlayerQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, false, user.PlayerTable, user.PlayerColumn),
		)
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withPlayer: uq.withPlayer.Clone(),
...
func (uq *UserQuery) WithPlayer(opts ...func(*PlayerQuery)) *UserQuery {
	query := (&PlayerClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPlayer; query != nil {
		if err := uq.loadPlayer(ctx, query, nodes, nil,
			func(n *User, e *Player) { n.Edges.Player = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadPlayer(ctx context.Context, query *PlayerQuery, nodes []*User, init func(*User), assign func(*User, *Player)) error {
	...
	query.Where(predicate.Player(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PlayerColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_player
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_player" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_player" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
